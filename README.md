...
a String value today (`d` or `e` to stage, `y` to confirm — every mutation previews the exact
command first, and Read-only Mode still refuses it where it should). String editing opens your
`$EDITOR`, so a value that reads as JSON gets real syntax highlighting for free; other Redis types,
TTLs, rename and copy are coming next.
