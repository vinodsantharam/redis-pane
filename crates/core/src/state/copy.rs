...
/// a copied command that does not work is worse than no command at all.
pub fn redis_cli_command(target: &str, open: &OpenKey) -> String {
    ...
    let key = shell_quote(&open.name);
    let verb = match &open.value {
...
mod tests {
    ...
    fn the_command_matches_the_type_so_it_actually_runs() {
        ...
        assert!(
            redis_cli_command(target, &open_with("k", hash())).ends_with("HGETALL k"),
            "GET on a hash is an error"
        );
        assert!(
            redis_cli_command(
                target,
                &open_with("k", Value::Str(StringValue::new("v", 8)))
            )
            .ends_with("GET k")
        );
        assert!(
            redis_cli_command(target, &open_with("k", Value::ZSet(ScoredValue::default())))
    ...
    fn the_command_carries_the_host_port_and_database() {
        let cmd = redis_cli_command("cache-01:6380/3", &open_with("k", hash()));
        assert_eq!(cmd, "redis-cli -h cache-01 -p 6380 -n 3 HGETALL k");
    }

    #[test]
    fn a_url_target_is_passed_through_as_a_url() {
        let cmd = redis_cli_command("redis://cache-01.eu-w1:6379", &open_with("k", hash()));
        assert!(
